...
import { validateEffectiveProjection } from '../scripts/inspect-systemd-effective.mjs';
...
function fixture() {
  ...
  return {
    ...
    socket: {
      ...
      Listen: '127.0.0.1:8405 (Stream)', Accept: 'no', Service: 'wallet-kernel.service',
...
test('real systemctl execution status is accepted but never changes the stable configuration hash', () => {
  ...
  for (const [code, status] of [['exited', '0'], ['killed', '15/TERM'], ['dumped', '11/SEGV']]) {
    ...
    for (const field of ['ExecStartPreEx', 'ExecStartEx']) {
